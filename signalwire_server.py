...
# Fallback / Twilio credentials (if ever needed)
...
# ---- Build telephony client (SignalWire default) ----
...
if PROVIDER == "signalwire" and SW_PROJECT_ID:
    ...
    print(f"[SWIFT] Telephony Provider: SignalWire | Bridge: {active_phone_number} | Space: {SW_SPACE}")
    ...
    print(f"[SWIFT] Telephony Provider: Twilio | Bridge: {active_phone_number}")
else:
...
print("[SWIFT] PhysioSpecNet Model ready.")

# ---- FastAPI App ----
app = FastAPI(title="SWIFT SignalWire Forensic Telephony Server", version="2.0.0")
...
# =============================================================================
# Phase 1 — SignalWire Inbound Webhook (Compatibility with SWML / cXML)
# POST /signalwire/inbound  (and aliased /twilio/inbound)
# Triggered when a call dials the SWIFT SignalWire bridge number.
# =============================================================================

@app.post("/signalwire/inbound")
@app.post("/twilio/inbound")
async def signalwire_inbound(request: Request):
    """
    Webhook — instructs SignalWire to:
    ...
    stream_url = f"{SWIFT_PUBLIC_URL.replace('https://', 'wss://')}/signalwire/stream"
...
@app.post("/signalwire/status_callback")
@app.post("/twilio/status_callback")
async def signalwire_status_callback(request: Request):
    return Response(content="OK", media_type="text/plain")


# =============================================================================
# Phase 2 — SignalWire Media Stream WebSocket
# WS /signalwire/stream  (and aliased /twilio/stream)
# Receives continuous base64 μ-law G.711 telephony audio packets.
# =============================================================================

@app.websocket("/signalwire/stream")
@app.websocket("/twilio/stream")
async def signalwire_stream(websocket: WebSocket):
    """
    SignalWire Media Streams WebSocket handler.
    ...
    print("[SWIFT] SignalWire Media Stream WebSocket connected.")
    ...
    try:
        async for raw_message in websocket.iter_text():
            ...
            # --- Connected event ---
            if event == "connected":
                print(f"[SWIFT] Media stream connected: protocol={msg.get('protocol')}")

            # --- Start event: gives us CallSid and StreamSid ---
            elif event == "start":
            ...
            # --- Media event: contains the actual audio ---
            elif event == "media":
                if session is None:
                    continue
        ...
        print("[SWIFT] Media Stream WebSocket disconnected.")
    except Exception as e:
        print(f"[SWIFT] Error in media stream handler: {e}")
    finally:
        if session:
    ...
    try:
        async for raw_msg in websocket.iter_text():
            ...
            if data.get("action") == "disconnect":
                ...
                if session.call_sid and telephony_client:
...
# GET /signalwire/verdict/{call_sid}  (and aliased /twilio/verdict/{call_sid})
# =============================================================================

@app.get("/signalwire/verdict/{call_sid}")
...
# POST /signalwire/action/{call_sid}  (and aliased /twilio/action/{call_sid})
# =============================================================================

@app.post("/signalwire/action/{call_sid}")
@app.post("/twilio/action/{call_sid}")
async def trigger_action(call_sid: str, request: Request):
    """Trigger a telephony action on a live call (warn or sever)."""
...
async def _trigger_mitigation(call_sid: str, action: str = "warn") -> dict:
    """Execute REST API mitigation on an active call via SignalWire."""
